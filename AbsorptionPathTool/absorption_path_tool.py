...
#
# This tool is released in conjunction with the E-XQR-30 metal absorber catalog published by
# Davies et al. (2023a)
...
# Calculate the relativistic velocity difference between an absorber at z = absorber_redshift and
# the QSO at z = qso_redshift
...
class AbsorptionPath():
    ...
    def convert_z_to_dX(self, z: float):
        # Calculate the absorption distance to an object at a redshift z. 
    ...
    def get_z_masked_regions(self, qso_id: str):
        ...
        # self.z_covered_bool is a dictionary where each key is a QSO ID and each value is a numpy
        # array. For each index i, self.z_covered_bool[qso_id][i] will either set to 1 if the
        # redshift given by self.z_ref_array[i] is covered by the absorption search path for the
        # relevant quasar and ion/transitions, or 0 otherwise.
        ...
        #
        # When get_z_masked_regions is called as a method on AbsorptionPathTransitions,
        # self.line_combo_list contains a single list which is the provided list of transitions, and
        # only one loop will be executed. In this case, the absorption path will not include
        # wavelength regions for which ANY of the transitions lie in one of the excluded regions
        # (e.g. BAL or masked regions).
        #
        # When get_z_masked_regions is called as a method on AbsorptionPathPrimaryIon;
        # - if the ion is one of the doublets (MgII, FeII, CIV or SiIV), self.line_combo_list
        #   contains a single list of the two doublet lines, and only one loop will be executed
        # - if the ion is either FeII or CII, self.line_combo_list contains multiple lists
        #   representing all two-transition combinations of the transitions listed for the relevant
        #   ion in search_transitions_dict. In this case, the absorption path will only exclude bad
        #   wavelength regions for which at least one transition of ALL line combinbations lies in
        #   the relevant region.
        for line_combo in self.line_combo_list:
            ...
            # The minimum redshift where this combination of transitions detected is the redshift
            # where the lowest wavelength line falls at the wavelength of the quasar Lya line.
            ...
            for exclude_wl_range in exclude_wl_ranges:
                # For each excluded wavelength range, the minimum redshift to exclude is given by
                # the redshift where the shortest wavelength line enters the lower end of the masked
                # region, and the maximum redshift to exclude is given by the redshift where the
                # longest wavelength line exits the upper end of the masked region.
            ...
            if len(exclude_redshift_intervals) > 0:
                ...
                while np.any(mask == 1):
                    # Go through the mask, identify contiguous regions where the mask is set to one,
                    # and append them to a list of covered redshift windows.
                    ...
                    # NOTE: Very important to use >= and <= here, otherwise the masking doesn't work
                    #       at all!
                    mask[np.logical_and(redshift_range >= z_start, redshift_range <= z_end)] = 0
                
                if len(covered_z_windows) == 0:
                    # If no windows have been added, it means that none of the region between
                    # [z_min, z_max] has been excluded, so include it all.
                ...
                # If no regions are excluded, then set the entries across the whole queried redshift
                # range to 1.
    ...
    # Use the self.z_covered_bool array to determine the redshift search intervals for the given
    # quasar and ion/transitions.
    ...
    def split_probed_zrange(self, config = {}):
        ...
        # NOTE: the sampling here needs to be SMALL, otherwise the division between the bins will
    ...
    def get_path_length_weighted_mean_redshift(self, bin_boundaries, config = {}):
        ...
        for (zlower, zupper) in bin_boundaries:
            # NOTE: The sampling here needs to be SMALL, otherwise the division between the bins
...
class AbsorptionPathTransitions(AbsorptionPath):
    def __init__(self, 
        # A list of transitions to compute the absorption search path for.
        transitions: List[str],
        # Configuration dictionary containing customizable parameters.
        config: Dict
    ):
...
class AbsorptionPathPrimaryIon(AbsorptionPath):
    def __init__(self,
        # The ion to compute the absorption search path for.
        ion: str,
        # Configuration dictionary containing customizable parameters.
        config: Dict
    ):
        ...
        # Transitions that were used when peforming search for absorption systems in Davies+2023a
        ...
        transition_list: List[str] = search_transitions_dict[ion].split(',')
        if ion == 'CII':
...
# Function to compute the absorption path covered by a specific subsets of quasars from the E-XQR-30
# survey for a given list of transitions.
...
# Function to compute the absorption path covered by a specific subsets of quasars from the E-XQR-30
# survey for one of the primary ions on which the absorber catalog is based (MgII, FeII, CII, CIV,
# SiIV and NV).
...
# Functions to compute the redshift boundaries for nbins bins that split the absorption search path
# evenly in either dX or dz.
...
# Functions to compute the path-length-weighted mean redshift for a set of redshift bins defined by
# the bin_boundaries list.
